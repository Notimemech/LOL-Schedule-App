...
export const createVNPayUrl = async (amount, ipAddr, userId) => {
    if (!amount || amount <= 0) {
    ...
    const orderId = `${userId}_${date.getTime()}`;

    await walletRepository.createVnpayPayment(userId, orderId, amount);
